...
if($_SERVER['REQUEST_METHOD'] == 'GET')
{
    $id = $_GET['id'];
    $dbo = new Database();
    $query = $dbo->conn->prepare("SELECT * FROM course_registration WHERE id = ?");
    try {
        $query->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['sid']) && isset($_POST['cid']) && isset($_POST['seid'])) {
        ...
        $id = (int)$_POST['id'];
        $dbo = new Database();
        $query = $dbo->conn->prepare("UPDATE course_registration SET student_id = ?,course_id = ?,session_id = ? WHERE id = ?");
        try {
            // echo var_dump($id);
            $query->execute([$sid,$cid,$seid,$id]);
            header('Location:../../adminCourseReg.php');
            exit();
        } catch (\Throwable $th) {
            //throw $th;
...
<head>
    ...
    <title>Update Course Registration </title>
        ...
        <div class="col-md-12">
            <div class="row">
                ...
                <div class="col-md-4 jumbotron my-5">
                    <h4 class="text-center my-2">Edit Course  Registration Details</h4>
                    <form method="post" action="edit_creg.php">
                        ...
                        <input type="submit" class="form-control btn btn-success" autocomplete="off" value="Edit Course Registration">
